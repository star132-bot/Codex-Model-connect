...
public sealed class ProviderRecord
{
    ...
    [JsonIgnore] public bool CanActivate => Enabled && VerifiedAt is not null && Protocol == "responses";
...
public sealed class WindowsState
{
    ...
    public string ConfigurationMode { get; set; } = "isolatedProfile";
...
public sealed class ManagerService
{
    ...
    public string ActiveTitle => State.ActiveProviderId == "openai" ? $"当前：本地 GPT · {State.OpenAIModel}"
        : $"当前：{(State.ConfigurationMode == "desktopMenu" ? "桌面新任务" : "外部 config_out")} · {State.Providers.FirstOrDefault(p => p.Id == State.ActiveProviderId)?.Name} · {State.ActiveModel}";
    ...
    public void SetMode(string mode) { State.ConfigurationMode = mode; Sync(); }
    ...
    private void Sync()
    {
        ...
        WriteExternalProfile(importable.Where(provider => !provider.IsAntigravity).ToList());
        if (State.ConfigurationMode == "desktopMenu") WriteDesktop(importable); else RemoveDesktop();
        Save();
    }
    ...
    private void WriteDesktop(List<ProviderRecord> providers)
    {
        if (providers.Count == 0) { RemoveDesktop(); return; }
        State.RouterSecret ??= Guid.NewGuid().ToString("N");
        var active = providers.FirstOrDefault(p => p.Id == State.ActiveProviderId);
        WriteCatalog(Paths.ActiveCatalog, providers, true);
        AtomicWrite(Paths.RouterState, JsonSerializer.Serialize(State, JsonOptions()));
        ...
        text = Toml.SetTop(text, "model", active?.ModelId ?? State.OpenAIModel);
        text = Toml.SetTop(text, "model_provider", "cmm_model_router");
        text = Toml.SetTop(text, "model_catalog_json", Paths.ActiveCatalog);
        text = text.TrimEnd() + $"\n\n# >>> codex-model-manager:providers\n# Only the local bridge lives in config.toml.\n\n[model_providers.cmm_model_router]\nname = \"Codex Model Router\"\nbase_url = \"http://127.0.0.1:17876/v1\"\nwire_api = \"responses\"\nrequires_openai_auth = true\nsupports_websockets = false\nhttp_headers = {{ X-Codex-Model-Manager-Token = \"{State.RouterSecret}\" }}\n\n# <<< codex-model-manager:providers\n";
        AtomicWrite(Paths.Config, text); RouterStartup.Enable();
    }
    ...
    private void RemoveDesktop()
    {
        var text = File.Exists(Paths.Config) ? File.ReadAllText(Paths.Config) : "";
        text = Toml.RemoveBlock(text, "# >>> codex-model-manager:providers", "# <<< codex-model-manager:providers");
        if (Toml.GetTop(text, "model_provider") == "cmm_model_router") text = Toml.RemoveTop(text, "model_provider");
        if (Toml.GetTop(text, "model_catalog_json") == Paths.ActiveCatalog) text = Toml.RemoveTop(text, "model_catalog_json");
        text = Toml.SetTop(text, "model", State.OpenAIModel);
        AtomicWrite(Paths.Config, text); RouterStartup.Disable(); State.RouterSecret = null;
    }
